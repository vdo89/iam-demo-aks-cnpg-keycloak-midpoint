...
2. The workflow will:
   ...
   - Reconcile **CNPG** cluster `iam-db` (+ Azure Blob backup config) via Argo CD. Update
     ...
     backups.
  - Configure CloudNativePG **managed roles** together with a declarative **Database** resource so the `keycloak` and `midpoint` users, databases, and required extensions (`pgcrypto`, `pg_trgm`) are provisioned from Git-managed secrets when the cluster bootstraps.
